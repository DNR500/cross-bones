...
const createFromTemplate = (template, renderModel) => new Promise((resolve, reject) => {
    template.render(renderModel, (err, output) => {
        if (err) {
            reject(err);
            return;
        }
...
const generateHTML = (filepath, fileOutline) => {
    return gatherBundlesListing(fileOutline.renderModel)
...
glob(srcPath, function (err, filepaths) {
    if (err) logError(err);
    const htmlList = filepaths.map(requirePath => {
        ...
        const establishFileOutline = require(path.relative(__dirname, requirePath));
        return establishFileOutline().then(generateHTML.bind(null, outputFilepath));
    });
...
const deleteCompiledMarkoFiles = (srcPath) => {
    const markoFilesPattern = `${srcPath.match(/^.*(?=(\.html.js))/)[0]}.marko.js`;
    glob(markoFilesPattern, function (err, filepaths) {
