...
const hasCorrectHTMLPageTitle = () => establishEntity()
        ...
        .end()
        .then((titleText) => {
            expect(titleText).to.equal('INTEGRATION');
        });

const initialPageLooksCorrect = () => takeScreenshot(establishEntity(),
                                        `${process.cwd()}/test/casper/screenshot/actual/fullscreen.png`, 'body')
...
const questionAnswersLooksCorrect = () => takeScreenshot(establishEntity(),
                                        `${process.cwd()}/test/casper/screenshot/actual/questionAnswers.png`, '.question-body')
        .then(checkExpectedFileExists.bind(null))
        .then(compareImages.bind(null))
        .then(comparisonData => {
            expect(comparisonData.isSameDimensions).to.be.true;
